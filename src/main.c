...
void key_callback(int key, int action, int mods)
{
    ...
    if (key  == conf.keys[KEY_RESET_CUBE_ROTATION].key && 
        mods == conf.keys[KEY_RESET_CUBE_ROTATION].mod && action == KEY_PRESS) {
        
        text_opacity = 255.f;
        strcpy(action_str, "RESET");
        text_opacity_anim = animate_scalar(&text_opacity, 0.f, conf.text_fading_duration, ease_out_sine);

        rubiks_cube_reset_rotation(rc);
    }
...
void render_frame(void)
{
    ...
    if (animation_is_running(&text_opacity_anim)) {
        ts = ww*4e-4;
        tw = get_text_width(ts, action_str);
        th = get_text_height(ts, action_str);
        tp = vec2((ww - tw)*0.5f, wh*0.85 + th);
        tc = color_from_rgba(255, 68, 18, text_opacity);
        render_text(tp, ts, tc, action_str);
    }
}